...
class Deck (val cards: MutableList<Card> = ArrayList()) {
    val boardCards: MutableLiveData<MutableList<Card>> = MutableLiveData(mutableListOf()) // LiveData for board cards
    ...
    fun dealCardsToBoard() {
        if (cards.size < 4) throw IllegalStateException("Not enough cards to deal 4 to the board, only ${cards.size} left")
        val dealtCards = mutableListOf<Card>()
        repeat(4) {
            val card = deal()
            dealtCards.add(card)
        }
        // Update LiveData with the new board cards
        boardCards.value = dealtCards
    }
